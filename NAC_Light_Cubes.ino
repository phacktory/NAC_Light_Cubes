...
int baseline = 40;                    // Audio noise level 
...
const int noise[]             = {1, 1, 1, 1, 1, 1, 1};    // set this to magnitude of noise
...
void spectrum() {
  ...
  for(int band = 0; band < 7; band++) {
    ...
    // Convert reading into brighness level
    ...
    // Filter out data below baseline
    if (numON[band] < baseline) {
      numON[band] = 0;
    }
